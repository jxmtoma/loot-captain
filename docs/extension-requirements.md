...
Profiles do not require a RaidLoot account. A profile can be created manually or imported from a RaidLoot profile (the default path) or an EverQuest `/output inventory` file. When an existing profile item has a RaidLoot item ID or name but no saved stats, the extension loads and saves the item stats when the profile opens or comparison starts.
...
6. Opening an existing profile shows fetched numeric stats in its item editor and saves them to that profile locally.
...
- The first comparison for imported items without saved stats may take a few seconds while RaidLoot item pages are fetched.
